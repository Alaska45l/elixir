...
export const GET: RequestHandler = async ({ fetch, url }) => {
  ...
  try {
    do {
      ...
      offset += limit;
    } while (products.length < total && products.length < 1000);
  } catch {
  ...
  const body = `<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${urls.map((path) => `<url><loc>${xmlEscape(`${site}${path}`)}</loc></url>`).join('')}</urlset>`;
